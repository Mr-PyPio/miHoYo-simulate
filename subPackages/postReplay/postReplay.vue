	...
	<view class="postReplayContent">
			...
			<view class="reviewsTop fixed" :style="{'top' : fixTop - 2 + 'px','display': reviewsTopFixed}">
				...
				<view class="topRight" @tap.stop="showSort($event,'right')" v-show="reviewsTop.rightTop">
					...
					<image src="../../static/poseRequlay/screen.png" mode="heightFix"></image>
				</view>
			</view>
...
<script>
	import {postFullApi,upvoteApi,follow,unfollow,collectPost,emotionApi,emotionKeywordsApi} from "../../common/api.js"
		...
		methods: {
			...
			setHiddenContainerStyle(height) {
				...
				this.$nextTick(function() {
					...
					console.log(this.miniHeight,height)
					if(this.diffHeight > 0) {
						this.popupHeight = this.windowHeight - this.miniHeight / this.rpxNum
						this.fixTop = this.miniHeight / this.rpxNum
					}else{
						this.popupHeight = this.windowHeight - height / this.rpxNum
						this.fixTop = this.initHeight / this.rpxNum 
					}
			...
			async getEmotionData() {
				...
				console.log(emotion,emotionKey)
				if(emotion) {
					this.emotion = emotion
				}else{
					const {data: res1} = await emotionApi()
					uni.setStorageSync('emotion',res1.data)
				}
				if(emotionKey) {
			...
			initEmotionData() {
				...
				const emotionKey = uni.getStorageSync('emotionKey')
				this.emotionKey = emotionKey
				if(!emotion || !emotionKey) {
					this.getEmotionDqta()
				}
			},
			async getEmotionDqta() {
				const {data: res1} = await emotionApi()
				this.emotion = res1.data
				uni.setStorageSync('emotion',res1.data)
				const {data: res2} = await emotionKeywordsApi()

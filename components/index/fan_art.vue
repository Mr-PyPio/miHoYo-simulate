...
<scroll-view class="itemContent" @scroll="scroll" @scrolltolower="scrollBottom"
	...
	<view class="fanArtContent" v-if="!pageLoading">
		<view class="topNoWrapp">
			...
			<view class="topNoList"  @touchmove.stop="tochMoveStop" v-if="topNoListData.length">
				<swiper display-multiple-items="3">
					<swiper-item  v-for="(item,index) in topNoListData" :key="index">
...
<script>
	import {fanArtTopNApi,fanArtMainApi,fanArtPostList} from '../../common/api.js'
		...
		methods: {
			...
			async getForumPostList(page = 1,page_size = 20,sort_type = 1,is_hot = false,is_up = 0,callback) {
				...
				if(page > 1 && this.last_id) {
					const {data: res} = await fanArtPostList(page,page_size,sort_type,is_hot,this.last_id)
					list = res.data.list 
					this.last_id = res.data.last_id
				}else{
					const {data: res} = await fanArtPostList(page,page_size,sort_type,is_hot)
					list = res.data.list 
					this.last_id = res.data.last_id
				}
				if(is_up === 1) {
			...
			openSortSelect(e) {
				const x = 15
				const y = e.detail.y
				uni.$emit('openWrap',this.sortConfig,x,y,'F')
			},
			scroll(e) {
		...
		.topNoList{
			...
			.topNoListItem{
				...
				.itemLogo{
					...
					top: 0;
					left: 0;
					padding: 6rpx 14rpx 6rpx 40rpx;
					background-color: #ffe067;
					border-radius: 14rpx 0 20rpx;
					background-image: url("../../static/fanNoTopIcon.png");
